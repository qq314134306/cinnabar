...
## D051 - Palace-Origin Transformations Stay Engine-Owned

When a natal palace is selected, Cinnabar exposes the four destination palaces
generated by that palace's heavenly stem in canonical Lu/Quan/Ke/Ji order. The
source palace must be resolved through iztro's functional `chart.palace()` API,
and destinations must come from that object's `mutagedPlaces()` result. Reading
the raw `chart.palaces` array is insufficient because it bypasses the engine's
