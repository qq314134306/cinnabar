...
- `app/src/lib/question-divination.ts` + its fixture/test and
  ...
  offline development sample, IANA wall-clock conversion, and the independently
  lazy Question Charts surface. Production currently returns only three
  versioned `ENGINE_UNAVAILABLE` results; the initial deterministic calculators
  are incomplete development placeholders and are not called or displayed.
  No synthesis, AI narrative, score, persistence, or remote fallback exists.
  See `docs/dev/question-divination-contracts.md`.
...
- `app/src/lib/compatibility-score.ts` +
  ...
  relationships, an eight-entry visible-stem map, and a directional hidden-stem
...
- `app/src/components/share/ShareCard.tsx` + `ShareCard.test.ts` - local
  ...
  AI narrative is only an optional quote source. Custom copy uses a separate
...
- `app/src/components/kline/LifeKLine.tsx` + `ScoreRadar.tsx` - visible English
  ...
  is requested only after the user builds a timeline. Local calculation
