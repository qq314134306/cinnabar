...
- **Your Chart**：輸入出生資料與地點，依需要套用真太陽時校正，再由 `iztro`
  ...
  而非孤立單宮閱讀。同一解讀面板另行列出本宮左右相鄰的兩個鄰宮，補充
  「夾宮」結構脈絡；鄰宮不會混入三方四正標示，也不會自動判為助力或阻力。
  生年四化索引會集中列出祿、權、科、忌對應的星曜與
  宮位，選取後直接開啟該宮及其三方四正；索引只整理命盤結構，不獨立判定
  吉凶。選取宮位後，同一面板也會依該宮天干列出祿、權、科、忌飛往的四個
  目標宮位並可直接跳轉；落宮取自 `iztro` 功能宮位介面，缺失結果維持不可用。
  大限與流年結構鏡頭可切換 1–100 歲模型中的年份，在本機疊加目前
  十年大限、流年命宮與兩層四化，並可從任一落點返回本命宮位及三方四正；
  此範圍只用於結構瀏覽，不預測結果或壽命。
